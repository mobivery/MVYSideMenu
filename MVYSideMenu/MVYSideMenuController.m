...
- (void)viewDidLoad
{
    ...
    if (_options.blurRadius > 0) {
        [self performSelector:@selector(snapShot) withObject:nil afterDelay:.5f];
        _background = [[UIImageView alloc] initWithFrame:_menuContainerView.frame];
        _background.contentMode=UIViewContentModeLeft;
        [_background setClipsToBounds:YES];
        [self.view insertSubview:_background belowSubview:_menuContainerView];
    }
...
- (UIImage *)blurWithCoreImage:(UIImage *)sourceImage
{
    ...
    [gaussianBlurFilter setValue:[NSNumber numberWithFloat:_options.blurRadius] forKey:@"inputRadius"];
...
- (void)snapShot{
    UIImage *back = [self takeSnapshotOfView:_contentViewController.view];
    UIImage *blurBack = [self blurWithCoreImage:back];
    [_background setImage:blurBack];
}
...
- (CGRect)applyTranslation:(CGPoint)translation toFrame:(CGRect)frame {
    ...
    //blur background
    CGRect bFrame = _background.frame;
    bFrame.origin.x = 0;
    bFrame.origin.y=0;
    bFrame.size.width = _menuContainerView.frame.size.width + newOrigin;
    _background.frame=bFrame;
